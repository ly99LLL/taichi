...
- **常驻粒子模型**：7200 粒子始终存在，不在手掌位置批量生成或销毁。
- **相反旋向与弹性碰撞**：左右涡场反向旋转，靠近时接触区会排斥、擦边并甩散粒子。
...
| GPU | 建议 NVIDIA CUDA；CPU / Metal / Vulkan 也可用，性能见下表 |
...
| 硬件 | 推荐后端 | `--arch` 值 | 性能 |
|---|---|---|---|
| NVIDIA GPU（Windows / Linux） | CUDA | `cuda` 或 `auto` | ⚡ 最佳 |
| Apple Silicon Mac（M1–M3） | Metal | `auto` | ✅ 良好（自动选择 Metal） |
| Intel Mac | CPU | `cpu` | 🐢 可用但较慢 |
| Intel 核显 / AMD GPU（Windows / Linux） | Vulkan | `vulkan` 或 `auto` | ✅ 取决于驱动 |
| 无 GPU / 虚拟机 | CPU | `cpu` | 🐢 可用但较慢 |

> CPU 模式可以运行所有功能（实时摄像头、视频渲染、合成演示），
> 但 7200 粒子的实时帧率会明显低于 GPU 模式。
...
无 NVIDIA 显卡或使用 Mac 时，加上 `--arch` 参数明确后端：

```bash
python -m yan_gua --arch cpu       # 强制 CPU 模式
python -m yan_gua --arch auto      # 自动检测（默认，Mac 会选 Metal）
```

**Windows 用户**也可以在依赖安装完成后双击 `run.bat`。**macOS / Linux 用户**可使用：

```bash
bash run.sh
bash run.sh --arch cpu
```

启动器会优先使用已有的 `JAVA_HOME`，否则从 `PATH` 查找 Java，不会覆盖系统配置。
...
```bash
python -m yan_gua               # 自动选择 GPU 后端
python -m yan_gua --arch cpu    # CPU 模式（无 NVIDIA 显卡 / Mac）
python -m yan_gua --arch metal  # 强制 Metal（Apple Silicon）
```

**macOS 用户注意：** 首次运行摄像头模式时，系统会弹出权限对话框，需允许终端（或
IDE）访问摄像头。Intel Mac 建议使用 `--arch cpu`。
...
- `docs/assets/demo.png`：README 封面截图。
...
| 参数 | 默认值 | 作用 |
|---|---:|---|
| `PARTICLE_COUNT` | 7200 | 常驻尘埃数量 |
| `VORTEX_ORBIT_RADIUS` | 92 | 稳定轨道半径 |
| `VORTEX_INFLUENCE_RADIUS` | 320 | 涡场影响外缘 |
| `VORTEX_SLOW_SPEED` | 135 | 开始失去相干的速度 |
| `VORTEX_BREAK_SPEED` | 480 | 完全解旋的速度 |
| `VORTEX_FORM_SECONDS` | 0.24 | 新涡旋成核时间 |
| `VORTEX_ECHO_SECONDS` | 2.4 | 余涡衰减时间尺度 |
| `VORTEX_POSITION_RESPONSE_SECONDS` | 0.024 | 涡心跟手响应时间 |
| `VORTEX_HAND_CARRY` | 0.78 | 慢速时粒子继承手速的比例 |
| `VORTEX_STOP_SPLASH_SPEED` | 170 | 急停泼洒触发速度 |
| `FAST_LOST_PREDICT_SECONDS` | 0.22 | 高速丢检后的短时预测补位时长 |
| `FAST_LOST_PREDICT_SPEED` | 155 | 启用高速预测补位的最低手速 |
...
- 实时渲染主要面向 Windows/Linux + NVIDIA CUDA。CPU 模式可用但帧率较低；
  Apple Silicon Mac 的 Metal 后端有良好性能。
- macOS Intel 机型无法使用 Metal 后端，`--arch auto` 会自动退回 CPU。
